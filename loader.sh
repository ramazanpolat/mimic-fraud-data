  ...
  if [[ $file_count -gt 0 ]]; then
    for file in $(ls output/*.load 2>/dev/null); do
      ...
      if [[ "$file" == *fraud* ]]; then
        printf 'loading %s ...' "$file"
        # asagidaki satiri kendinize gore degistiriniz
        docker run -i --rm --link clickhouse:clickhouse yandex/clickhouse-client:20.8.12.2 --host clickhouse -q "insert into fraud_result(record_date,mbb,device_uid,query_name,source,package_list,device_client_id,device_info,type) FORMAT JSONEachRow SETTINGS max_insert_threads=16" < $file
        printf 'done.\n'
        sleep 0.1
      fi
      echo "deleting $file"
